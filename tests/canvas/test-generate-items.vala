...
public class Akira.GenerateItems : Akira.TestSuite {
    ...
    public void loop_through_items () {
        app.activate.connect ((a) => {
            ...
            for (var i = 0; i < 1000; i++) {
                ...
                // canvas.selected_bound_manager.add_item_to_selection (item as Lib.Items.CanvasItem);

                // TRUE: We selected the correct item.
                // assert (canvas.selected_bound_manager.selected_items.index (item) == 0);
                // assert (((Lib.Items.CanvasItem) item).layer.selected == true);
